...
class FFTDecomposition(nn.Module):
    def __init__(self, freq_cutoff=0.1):
        """
        freq_cutoff: 상대 주파수 컷오프 (0~0.5 사이)
        """
        super(FFTDecomposition, self).__init__()
    ...
    def forward(self, obs):
        ...
        for b in range(B):
            for c in range(C):
                arr = obs[b, :, c].detach().cpu().numpy()
                # FFT
                fft_coeffs = np.fft.fft(arr)
                freqs = np.fft.fftfreq(L)
                # low-pass 필터링
                mask = np.abs(freqs) <= self.freq_cutoff
                fft_low = fft_coeffs * mask
                # 역변환 → trend
                trend_np = np.fft.ifft(fft_low).real
                # Tensor로 복원
                trend[b, :, c] = torch.tensor(trend_np, device=obs.device, dtype=obs.dtype)
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.freq_cutoff = configs.freq_cutoff
        self.fft_decomp = FFTDecomposition(freq_cutoff=self.freq_cutoff)
    ...
    def forward(self, x):
        ...
        trend_obs, residual_obs = self.fft_decomp(x)
